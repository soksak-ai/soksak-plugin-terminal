...
// 앱(코어 theme/engine.ts)은 두 채널로 테마를 발행한다:
...
//
// 배경 규칙(코어 terminal/theme.ts 머리말): xterm theme.background 는 *불투명* 색이어야
...
// 그리드 배경 폴백 — --bg 가 비었을 때만. 코어 terminal/theme.ts 와 동일.
export const backgrounds: Record<ThemeMode, string> = {
  dark: "#1e1e1e",
  light: "#ffffff",
};

// 다크 — 16 ANSI + foreground/cursor/selection (VSCode Dark 계열). 코어 theme.ts 이식.
...
// 라이트 — 흰 배경 가독 팔레트 (VSCode Light 계열). 코어 theme.ts 이식.
...
// 앱이 발행한 배경 색(--bg). 코어 engine.ts 가 COLOR_SLOTS "bg" → documentElement.style.setProperty("--bg")
