...
// [SWAP: view-wrapper] 원본은 Vite 의 CSS 사이드이펙트 import("@xterm/xterm/css/xterm.css")로
// xterm CSS 를 주입했다. 플러그인은 styles.ts 가 esbuild loader:text 로 같은 CSS 를 문자열
// 번들 → injectStyles()(plugin-entry 가 활성화 시 호출)로 1회 주입한다. 그래서 여기선 생략.
...
// [SWAP: PTY IO] 원본은 invoke("write_text_file") 로 /tmp/soksak-ime.log 에 flush 했다.
// 플러그인 컨텍스트엔 Tauri invoke 가 없어 console.debug 로 대체한다 — DEV 전용 경로이고
// 빌드(import.meta.env.DEV=false)에서 통째로 제거되므로 동작 차이는 없다.
...
export interface CreateTerminalOptions {
  // [SWAP: PTY IO] 코어 PTY 표면(app.pty). 원본은 invoke()/Channel 직호출이었다.
  pty: PtyApi;
  cwd?: string;
  shell?: string;
  theme?: ITheme;
...
// [SWAP: settings] 원본은 core state/settings 의 TerminalSettings/XtermRenderer 를 import 했다.
// 플러그인은 manifest contributes.configuration 와 1:1 인 자기 소유 설정 타입을 쓴다.
...
// [SWAP: view-wrapper] 원본은 createTerminal(container, options) 으로 React TerminalView 가
// 넘긴 container 에 마운트하고 TerminalHandle 을 돌려줬다. 플러그인은 plugin-entry 가
// 마운트하므로 container 를 내부에서 만들고 element 를 instance API 로 노출한다.
...
export async function createTerminal(
  ...
  // [SWAP: PTY IO] pty 표면을 옵션에서 받는다(원본은 모듈 전역 invoke).
  ...
  const term = new Terminal({
    ...
    // [SWAP: theme-source] 원본은 options.theme ?? darkTheme. 플러그인은 앱 테마 계약
    // (documentElement.dataset.themeMode + :root --bg)을 themeFor() 로 상속한다.
  ...
  // [SWAP: PTY IO] 원본은 opener 플러그인 openUrl(). 플러그인 컨텍스트엔 없어 window.open 위임.
  term.loadAddon(
  ...
  // [SWAP: view-wrapper] 컨테이너 div 를 내부 생성(원본은 인자로 받음).
  ...
  // (폴링 없음). 원본은 코어 systemThemeWatcher 가 TerminalView 의 setTheme 를 호출했다 —
  // 플러그인엔 그 호출자가 없어 DOM 계약을 직접 관찰한다. setTheme 의 색 교체·아틀라스
  // 비우기 형태는 보존한다.
  ...
  // content-visibility:hidden 을 줘 recalc 에서 제외(렌더만 스킵, 상태 보존)하면서 해소됐다. 그래서 여기선
  ...
  let termId = 0;
  let ackPending = 0;
  // [SWAP: PTY IO] 언마운트가 spawn 보다 먼저 일어난 경우(async race)를 처리한다 — 원본은
  // TerminalView 가 동기로 container 를 쥐고 있어 race 가 없었지만, 플러그인은 spawn 을
  // await 하는 사이 unmount 가 올 수 있다.
  let disposed = false;
  ...
  const writeToPty = (data: string) => {
    ...
    if (termId !== 0) {
      // [SWAP: PTY IO] invoke("write_terminal", {id, data}) → pty.write(id, data).
  ...
  // [SWAP: observation-registerIo] 원본은 여기서 셸 통합(setupShellIntegration: OSC 133/633/7)을
  // xterm 에 물려 명령 데코레이션·cwd 추적을 했고, onCwd/onCommand 를 코어 paneHosts 로 브리지했다.
  // 플러그인은 xterm-측 관찰을 하지 않는다 — 관찰은 코어 PTY substrate(ptyObservationStore)가
  // PTY 바이트스트림에서 직접 생산하고, 플러그인은 app.pty.registerIo(viewId, {readBuffer,
  // sendInput}) 로 IO 만 등록한다(registerIo 배선은 plugin-entry 가 담당). 그래서 OSC 관찰
  // 브리징은 의도적으로 생략한다(이중 관찰 제거, 단일 진실=코어 substrate).
  ...
  // [SWAP: PTY IO] PTY 출력은 코어 substrate 의 app.pty.onData(id, cb) 로 구독한다(원본은
  // Channel<ArrayBuffer>). onData 는 스폰 전 출력도 버퍼링하므로 손실이 없다 — 단 ptyId 가
  // 확정된 뒤에만 등록할 수 있어, 원본이 spawn 인자로 Channel 을 넘긴 것과 달리 spawn 완료
  // 후 연결한다. ACK 플로우 컨트롤(5k 누적)·write 콜백 시점은 원본과 동일하게 유지한다.
    ...
    dataSub = pty.onData(termId, (bytes: Uint8Array) => {
      ...
      const doAck = () => {
        ...
        if (ackPending >= FLOW_ACK_SIZE && termId !== 0) {
          // [SWAP: PTY IO] invoke("ack_terminal", {id, bytes}) → pty.ack(id, bytes).
  ...
  // [SWAP: PTY IO] invoke("spawn_terminal", {...,onOutput:Channel}) → pty.spawn({...}) 반환 id.
  // windowLabel(원본의 currentWindowLabel())은 코어 substrate 가 내부에서 채운다 — 플러그인은
  // 자기 창 label 을 알 필요가 없다(코어가 IO 등록 키=paneId 로 라우팅).
  ...
  // [SWAP: PTY IO] spawn 을 await 하는 사이 unmount 가 온 경우(async race) — 즉시 닫는다.
  if (disposed) {
  ...
  const syncPty = () => {
    if (termId !== 0) {
      // [SWAP: PTY IO] invoke("resize_terminal", {id, cols, rows}) → pty.resize(id, cols, rows).
  ...
  const dispose = () => {
    ...
    // [SWAP: observation-registerIo] 원본은 shellIntegration.dispose(). 관찰을 안 하므로
    // 대신 PTY 출력 구독(dataSub)·테마 옵저버를 해지한다.
    dataSub?.dispose();
    themeObserver.disconnect();    visObserver.disconnect();
    if (termId !== 0) {
      // [SWAP: PTY IO] invoke("close_terminal", {id}) → pty.close(id).
      pty.close(termId).catch(() => {});
    }
    webgl?.dispose();
    term.dispose();
  ...
  // [SWAP: view-wrapper] 원본은 TerminalHandle(terminal/id/onCwd…/setTheme/…)을 반환했다.
  // 플러그인은 plugin-entry 가 기대하는 instance API(element/dispose/focus/sendInput/
  // readBuffer/clear/applySettings/fit/paste)로 노출한다 — 같은 동작, 다른 표면.
  return {
    element: container,
    ...
    applySettings: (next: TermSettings) => {
      ...
      // 전 필드 필수). 플러그인 TermSettings 는 전 필드 optional 이라, 들어온 값만 대입한다.
...
// [SWAP: view-wrapper] plugin-entry 가 호출하는 진입점 별칭. 원본엔 없던 이름이지만
// createTerminal 을 그대로 위임한다(시그니처/동작 동일) — plugin-entry 의 기존 배선 보존용.
